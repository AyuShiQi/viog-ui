<template>
  <span
  class="vi-select-item">
  <span>
    <slot></slot>
  </span>
  <svg
  class="vi-select-chacha"
  viewBox="0 0 1024 1024"
  version="1.1"
  xmlns="http://www.w3.org/2000/svg"
  @click="toDelete($event, data)">
...
export default defineComponent({
  ...
  setup (props: any, ctx: SetupContext) {
    const deleteSelect: ((item: any) => void) | undefined = inject('delete:choosed', undefined)

    function toDelete (e: Event, item: any) {
      if (deleteSelect) deleteSelect(item)
      e.stopPropagation()
    }

<template>
  <div class="vi-message" v-show="show">
    <transition name="vi-message" v-for="item in message" :key="item">
      <span
      class="vi-message-card">
        <span class="vi-message-icon"></span>
        <span class="vi-message-title">{{item}}</span>
        <span class="delete-point" @click="shutDown">
          <svg v-if="shutdown" class="icon" width="16px" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg">

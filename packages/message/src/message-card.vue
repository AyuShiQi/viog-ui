<template>
    <span
    class="vi-message-card"
    :class="{
      'vi-message-card_appear': appear,
      'vi-message-card_disappear': disappear
    }">
      <span class="vi-message-icon"></span>
      <span class="vi-message-title">{{message}}</span>
      <svg v-if="shutdown" class="delete-point" @click="shutDown" width="16px" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg">
...
export default {
  ...
  setup (props: any, ctx: SetupContext) {
    ...
    onMounted(() => {
      setTimeout(() => { appear.value = true })
      if (props.duration) setTimeout(() => { disappear.value = true }, props.duration - 200)
    })
